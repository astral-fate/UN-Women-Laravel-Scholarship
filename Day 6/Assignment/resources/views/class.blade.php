            ...
            <tr>
              ...
              <td>
                <form action="{{ route('class.destroy', $class['id']) }}" method="POST" style="display: inline;">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="btn btn-link p-0" style="color: #007bff; text-decoration: none;" onclick="return confirm('Are you sure you want to delete this class?')">Delete</button>
                </form>
              </td>
            </tr>
            @endforeach
